...
- Agent/host 写入合法 input 后如何进入 current、失败 attempt 如何记录、legacy project 如何恢复此前没有公共合同。
...
- `project.json.asset_usage_plan` 与 `edit-plan.json.asset_usage_plan` 作为长期并行入口并在 render runtime 合并，是重复且冲突的权威路径；它们只保留为一次性 legacy migration input。
...
简化的 `asset_usage_plan` 可以作为 platform handoff 或 legacy compatibility input，但不能长期同时存在于 `project.json`、`edit-plan.json` 和 render runtime 中形成平行权威路径。目标行为是：

1. 新工作流直接写 `enrichment-plan.json`；
2. 需要简化 handoff 时，使用单独、明确的 `asset-usage-plan.json` command input；
3. `project enrich-plan` 校验并把它一次性归一化为 canonical `enrichment-plan.json`；
4. 同时存在 canonical plan 与 legacy/简化 usage plan 时返回 conflict，不做隐式 merge；
5. render 只消费 current `enrichment-plan.json`；
6. 旧 `project.json.asset_usage_plan` 或 `edit-plan.json.asset_usage_plan` 只作为迁移输入，不能继续作为长期 render source。

| Artifact | 角色 | 作者 | 消费者 | 权威边界 |
| --- | --- | --- | --- | --- |
| `asset-usage-plan.json` | command_request | Skill/host | `project enrich-plan` | 简化/兼容素材交接，不直接进入 render |
...
本轮不搬迁整个 project layout，也不要求用户重建 source。

没有 `artifact-manifest.json` 的项目状态为 `legacy_untracked`。`project status` 应：

- 读取并校验现有 artifacts；
- 把 schema 合法的 agent/host authoritative inputs 标为 `pending_validation`；结构合法但 lineage 无法证明的旧 derived/result 标为 `stale`，reason/code 为 `LINEAGE_UNPROVEN`；
- 不把无法证明 lineage 的派生产物标成 current；
- 返回最早可执行 validator/consumer，例如 `project proposal`、EDL consumer 或 `project enrich-plan`；
- 允许后续正常命令逐步写入 manifest；
- 在新的 render 成功后建立完整 render lineage；
- 在新的 inspect 成功后建立 inspection lineage。

旧 Markdown、EDL、storyboard、MP4 和 report 可以物理保留，但在没有 current record 前不能证明成功。

旧的多入口 `asset_usage_plan` 兼容规则是：

- 只有一个 legacy source 且没有 canonical enrichment plan 时，可以由 `project enrich-plan` 归一化；
- 多个 legacy sources 同时存在，或与 canonical plan 同时存在时，返回明确 conflict；
- 不隐式合并；
- 归一化成功后，render 只消费生成的 canonical enrichment plan。
