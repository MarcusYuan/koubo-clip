...
- Agent/Host authored artifact 的结构合同由 CLI 版本化公开；Skill 只维护业务方法。安装后的 Agent 不应读取源码或靠连续 validator 失败来恢复 schema。
...
enrichment 层应表达为 2.0 output-timeline plan：

- `profile`: source mode、aspect ratio、caption identity、layout、style 和 frame。
- `elements[]`: `registry_block`、`registry_component`、`caption_identity`、`animation_rule` 或 `visual_asset`；生成素材也以带 provenance 的 `visual_asset` 表达。
- Caption/visual 进入唯一当前 2.0 `elements[]`，SFX/music 进入 `audio`；不保留 cards、slots 或其他旧 plan 输入。
- `audio.music[]`: background music intent、volume、fade 和 ducking policy；`audio.sfx[]` 表达 project/bundled SFX。
...
- 按用户最终决策，生成的 public workspace 只能加载 CLI allowlist 中锁定的 runtime，例如 `gsap@3.15.0`；Google Fonts family CSS 是明确记录的 versionless exception。
...
- 使用 vendored registry elements 和 deterministic CLI-owned HTML/SVG 生成 recut，不接受任意 agent-authored HTML。
...
Artifact schema 已经成为真实集成压力，因此 ownership、contract discovery、聚合 validation 和发布防漂移要求已进入 `rules/artifact-contracts.md`，不再属于候选规则。
