...
- Proposal 与 EDL compiler 必须复用同一 candidate-cleanup retained-range 算法和 overlay source-range resolver。当前边界语义使用既有 cut padding 与精确包含关系；相接边界合法，跨越删除区间非法。`compile-edl` 保留独立校验，防止旧 artifact、缓存篡改或绕过 proposal 的输入继续执行。
