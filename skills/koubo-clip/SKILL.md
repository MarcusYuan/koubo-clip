...
- Before user confirmation, `source-frame-request.json`, `source-frames.json`, and the CLI-managed source-frame JPEGs are the only allowed media-evidence exception. They are read-only source-understanding artifacts and do not imply approval; platform hosts access them only through an authorized public evidence surface.
